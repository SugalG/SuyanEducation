...
export default function NavigationBar({ settings, onApplyNow, isHomepage = false }) {
  ...
  const mobileMenuItems = [
    ...
    { href: "/universities", label: "Universities", icon: <Building size={20} /> },
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between text-xs sm:text-sm">
            ...
            <div className="flex items-center gap-2 sm:gap-3 md:gap-4">
              <div className="flex items-center gap-1 sm:gap-2">
                ...
                <span className="md:hidden text-xs truncate">Lalitpur, Nepal</span>
              </div>
            ...
            <Link href="/" className="flex items-center group">
              <div className="relative">
                <Image
                  ...
                  height={80}
                  priority
                  className="h-16 sm:h-20 lg:h-24 w-auto transition-transform duration-300 group-hover:scale-105 object-contain"
                ...
                {mobileMenuItems.map((item) => (
                  item.submenu ? (
                    <div key={item.label} className="border-b border-gray-100 last:border-0">
                      <button
                        onClick={() => setMobileSub(mobileSub === item.submenu ? null : item.submenu)}
